...
myrt$V_xsection_cm3_damaged_revised <- ((myrt$drill_dbh_cm * myrt$linear_mean)/2)^2 * pi
...
m.2 <- lmer(log(V_xsection_cm3_damaged_revised) ~ drill_height_cm + species + (1|tree_site_ID), REML = F, data = myrt)
#summary(m.2)
#check_model(m.2)
Anova(m.2)
...
m.4 <- lmer(log(V_xsection_cm3_damaged_revised) ~ drill_height_cm + dbh_cm.x + wd + (1|tree_site_ID) + (1|site), REML = F, data = myrt)
#summary(m.4)
#check_model(m.4)
Anova(m.4)
```
...
b <- ggplot(myrt, aes(x = species, y = V_xsection_cm3_damaged_revised, fill = species)) + 
  geom_boxplot() +
  geom_point()+ 
  theme_classic() + 
  scale_fill_brewer(palette = "Set3") +  
...
d <- ggplot(myrt, aes(x = drill_height_cm, y = V_xsection_cm3_damaged_revised, color = species)) +
  geom_jitter() +
