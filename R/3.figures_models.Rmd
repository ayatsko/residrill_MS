...
Anova(mod.B)
#check_model(mod.B)
dam_dbh <- ggpredict(mod.B, terms = c("dbh_cm"), back.transform = TRUE)
...
tr <-ggplot(frustrum, aes(x = dbh_cm, y = mp_damaged_kg)) +
  geom_jitter() +
  geom_line(aes(x=x, y=predicted), data=filter(dam_dbh), 
            inherit.aes=FALSE, colour= low) + 
  geom_ribbon(aes(x=x, ymin=conf.low, ymax=conf.high), data=filter(dam_dbh), 
              inherit.aes=FALSE,  alpha=0.2, fill = low)+
  theme_classic() + 
  xlab("Tree DBH (cm)") + 
  ylab("Total damaged biomass in lower stem (kg)")
...
term_dbh + term_wd + fire_dbh + fire_wd + patchwork::plot_annotation(tag_levels = "a") 
```
